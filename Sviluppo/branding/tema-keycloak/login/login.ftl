...
<#--
  ...
  Invariati rispetto al base, perche' toccarli rompe Keycloak:
    - nomi dei campi: username, password, rememberMe, credentialId, login
    - ordine dei campi e tabindex 2..7 (autofill e navigazione a tab)
    - gestione errori via messagesPerField.existsError('username','password')
    - action="${url.loginAction}" e method="post"
    - il pulsante nativo di visibilita' della password (data-password-toggle)
      con il suo passwordVisibility.js: e' di Keycloak, non del prototipo

  Il titolo viene reso qui, nella sezione "form", per poterlo mettere sotto il
  logo dentro la card. La sezione "header" resta quindi vuota e il suo <h1>
  viene nascosto dal CSS: e' l'unico modo di ottenere l'ordine logo-titolo
  senza copiare anche template.ftl, che raddoppierebbe la manutenzione.
-->
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
    <#elseif section = "form">
        <div id="az-intestazione">
            <div id="az-logo" role="img" aria-label="${msg('azLogoAlt')}"></div>
            <h1 id="az-titolo">${msg("loginAccountTitle")}</h1>
            <p id="az-sottotitolo">${msg("azSottotitolo")}</p>
        </div>
