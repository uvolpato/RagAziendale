...
| 55 | **Portale dopo il login** | Operatori: redirect server-side diretto in chat, nessuna pagina. Amministratori: schermata di scelta Chat / Amministrazione | L'operatore non deve vedere passaggi in più; l'amministratore ha due mestieri | 📝 `SPECIFICA-INTERFACCE-AMMINISTRAZIONE.md` |
| 56 | **Ruoli di amministrazione** | Ruoli atomici (accessi, fonti, importazioni, anomalie, sistemi, ruoli, revisore) aggregati in profili; ambito limitato alle aziende abilitate. In Keycloak: ruoli del client `amministrazione`, profili = gruppi con ruoli composti | Negli uffici chi gestisce gli accessi non è chi segue i dati; nessuno si alza i permessi da solo | 📝 idem §3 |
...
| 61 | **Come entrano i documenti** | **Cartelle collegate** (canale principale): ogni fonte è una cartella che l'ufficio già usa; l'indicizzazione la rilegge periodicamente (nuovi, modificati, cancellati). Caricamento dall'amministrazione: fase 2, per fonti senza cartella. Caricamento in chat: solo documenti personali e temporanei, fase 2, passando dall'orchestratore | La base aziendale richiede per ogni fonte gruppi, aziende, responsabile e versione valida: un allegato in chat non li ha | ✅ |
| 62 | **Connettori documentali multipli** | Ogni fonte dichiara il suo connettore (cartella, SharePoint/OneDrive, Google Drive, ...) dietro un'interfaccia comune: *elenca* (id, percorso, data modifica, impronta) e *leggi*. Il primo è **cartella** (percorso montato nel container; in produzione una condivisione di rete montata sull'host). Indicizzazione orchestrata da Dagster come le importazioni ERP | Le aziende tengono i documenti in posti diversi; il resto della pipeline (Docling, chunk, embedding, ACL) non deve sapere da dove arrivano | ✅ |
| 63 | **Caricamento file in chat disattivato** | `fileConfig` di LibreChat + blocco in Caddy su POST/PUT `/api/files*` + test 14 di `verifica_login.py` | Il sistema di file di LibreChat ha una ricerca propria che non passa dal gate (niente ACL, residenza, taint). `fileConfig.disabled` nasconde il pulsante ma non è dimostrato che il server lo applichi: la regola sta nel proxy | ✅ verificato: 403 anche via API |
| 64 | **Regole sui contenuti delle cartelle** | Nella cartella solo le versioni valide; `_archivio` escluso; un file cancellato sparisce dall'indice; listini e prezzi in Excel **non** si indicizzano (i prezzi vengono dall'ERP); file illeggibili → anomalia; il responsabile della fonte è l'ufficio che possiede i contenuti | Tre versioni dello stesso documento producono citazioni sicure e sbagliate; un prezzo da un foglio indicizzato come testo è il modo più rapido per sbagliare un preventivo | ✅ |
| 65 | **Deleghe di amministrazione in Keycloak** | Ruoli `admin-*` sul client `amministrazione`, profili come sottogruppi di `/amministratori`. `admin-accessi` **senza** ruoli di realm-management (scavalcherebbero le deleghe): permessi FGAP v2 su utenti (view, manage, manage-group-membership) e gruppi (view, view-members, manage-membership), con **divieto** sui gruppi di `/amministratori`. Nessun `reset-password` esplicito: Keycloak lo ricava da `manage`, quindi vale sugli operatori e non sugli amministratori. Applicate da `keycloak/deleghe.py` a ogni avvio; verificate da `eval/verifica_deleghe.py` (12/12). In produzione `/admin` di Keycloak va raggiungibile solo dalla rete interna | Con i soli ruoli classici chi gestisce gli accessi si aggiungeva al profilo Amministratore completo, si assegnava admin-ruoli e reimpostava la password di un amministratore (tutto verificato, poi chiuso) | ✅ |
| 66 | **Implementazione dell'amministrazione** | Servizio `Sviluppo/amministrazione/` (FastAPI + psycopg, SQL a mano) sotto `/amministrazione` sullo stesso host della chat. Client Keycloak **pubblico con PKCE**, sessioni in memoria con chiave per processo: **nessun segreto nuovo**. Il callback della chat atterra su `/amministrazione/dopo-login` (operatori → `/c/new` con un 302, amministratori → scelta). Permessi (§3.4) codificati una volta in `logica.py` e applicati alle API. Vedi come legge Keycloak **con il token dell'amministratore**. Migrazione 003: `sources` + tipo/provenienza/stato/aziende, anomalie deduplicate con `segnala_anomalia()`, registro immutabile per trigger, aspetto | Riusa lo stack esistente (decisione 42) senza segreti da custodire; un solo posto per i permessi; il registro vale come prova solo se nessuno può riscriverlo | ✅ verificato: 20/20 + 18/18 |
| 67 | **Gate: filtro per stato e azienda** — FATTA | `recupero.py` filtra solo `acl_groups && gruppi`. Stato della fonte (`attiva`) e aziende (decisione 53) vanno aggiunti al predicato del gate, con i test, **prima** di indicizzare dati di più aziende o di sospendere una fonte contando sul fatto che sparisca | Oggi una fonte «sospesa» resta interrogabile e l'asse azienda è solo dichiarato. Vedi come lo mostra come avviso, per non far sembrare chiuso un buco aperto | ✅ fatto: predicato in `recupero.py`, test T1.15–T1.17 (15/15) |
| 68 | **Un'interfaccia sola di amministrazione** — rivede in parte la 60 | Utenti, gruppi e profili si gestiscono **nel nostro pannello**, con le API di amministrazione di Keycloak chiamate **col token di chi lavora** (le deleghe le applica Keycloak). La console di Keycloak resta solo come «Configurazione avanzata» per il **Superutente** (ruolo `admin-super`, unico con ruoli classici `realm-admin`, tutte le aziende). Tutti gli altri ruoli: **solo deleghe fini**, mai ruoli classici (anche uno solo annullerebbe le deleghe di chi lo somma). I ruoli di amministrazione si danno **solo tramite profili**, mai diretti (i divieti proteggono i membri dei profili). Creare gruppi e comporre profili: solo Superutente. Il registro modifiche registra anche gli accessi | Due interfacce (pannello + console) disorientano chi amministra; uscire dalla console chiudeva l'SSO e lasciava il pannello a metà. Le API di Keycloak sono stabili, un tema della console no | ✅ verificato: deleghe 24/24, pannello 24/24 |
| 69 | **Ogni azienda ha il suo connettore** — rivede la 51 sulle chiavi | Un connettore per azienda verso il gestionale di riferimento, che importa tutto. Chiavi dei record `<azienda>:<id nel gestionale>` (spazio dei nomi = la nostra azienda, non il gestionale). `aziende`: connettore e codice nel gestionale facoltativi, nessun vincolo di unicità sul gestionale. Si amministrano dal pannello (Superutente): riga e gruppo Keycloak `azienda-<codice>` creati insieme, niente eliminazione (solo disattivazione con motivo). Credenziali solo in `.env` (migrazione 005) | Aziende del gruppo su gestionali diversi, o nello stesso gestionale con codici uguali: con la chiave sulla nostra azienda i dati non si mescolano mai. Il livello «collegamenti condivisi» era complessità senza un caso reale | ✅ verificato (Z1) |
| 70 | **Connettori e collegamenti** — PROPOSTA | Catalogo dei connettori nel codice (`Sviluppo/connettori/<tipo>/`: manifesto, interfaccia comune, kit di conformità). Collegamenti creati dal Superutente e abbinati alle aziende (unicità collegamento + codice azienda). Credenziali **cifrate nel database** (chiave in `.env`, sola scrittura, decifrate solo dal servizio `connettori`) — rivede la regola «credenziali solo in `.env`». Utente di sola lettura verificato. Motore di importazione comune (sosta, fusione SCD2, controlli, anomalie) orchestrato da Dagster | Un prodotto installato presso clienti diversi deve poter aggiungere un gestionale senza toccare il server; i connettori piccoli e uguali non possono rompere lo storico | 📝 `SPECIFICA-CONNETTORI.md`, da approvare |
| 71 | **Una cartella per gruppo, con i suoi gestori** | Ogni gruppo ha una cartella: chi è nel gruppo vede i documenti nell'assistente e li deposita nella cartella. Chi sta in `<gruppo>-gestori` aggiunge e toglie i colleghi del **solo** suo gruppo dal pannello (*I miei gruppi*) e risponde della cartella. Tutto ciò che è nella cartella si indicizza, tranne `_bozze`, `_archivio` e i fogli di calcolo; tutta la cartella ha lo stesso gruppo (niente eccezioni per sottocartella: una sottocartella più riservata è la cartella di un altro gruppo). Una cartella generale letta da tutti e scritta da pochi è lo stesso modello con il gruppo `tutti`. Le cartelle si collegano da *Fonti → Nuova fonte → Cartella* e nascono in attesa. Il limite «solo il suo gruppo» lo applica Keycloak (deleghe FGAP, `amministrazione/gestori.py`); il permesso di **scrittura** sulla cartella lo applica chi ospita i file (D15) | L'ufficio che produce i documenti è quello che sa quali valgono; le persone cambiano più spesso delle regole, e passare dall'IT per ogni nuovo addetto non regge | ✅ cartelle locali e gestori (verifica_cartelle, verifica_deleghe); condivisione vera in D15 |
| 72 | **Fogli di calcolo: si leggono, tranne quelli con i prezzi** (precisa la 64) | Un foglio resta fuori se ha celle in formato valuta, o una colonna da soldi (*prezzo, listino, costo, sconto, netto, importo, tariffa, €*) con numeri sotto. Solo l'intestazione non basta: un registro dei rischi con «costo stimato: alto» entra. Resta visibile come «Escluso», con il motivo, nella scheda Documenti della fonte; `.xls`/`.ods` come esclusi da convertire. Nel dubbio (file che non si riesce a controllare) resta fuori | La 64 escludeva tutti i fogli per proteggere i prezzi, ma registri dei rischi, dei trattamenti e piani di formazione vivono in Excel. Un controllo sulle colonne è deterministico e gratuito; con il modello (D1) si potrà aggiungere un parere dell'LLM sui casi dubbi | ✅ `ingestion/indicizza.py`, test in `test_indicizza.py` e `verifica_cartelle.py` |
| 73 | **La GPU si divide con il modello di chat** | L'indicizzazione legge i documenti sulla GPU **solo se c'è posto mentre il modello di chat è caricato** (`VRAM_MINIMA_MB`, controllata prima di ogni blocco di pagine): il modello di chi sta chattando non si scarica mai per iniziativa del servizio. Dentro `FINESTRA_NOTTE` (e con `--forza`, che è una persona a chiederlo) il modello si scarica, l'embedding resta — serve a fare i vettori — e si legge tutto, anche i file oltre `MB_MAX_DI_GIORNO` che di giorno aspettano. Finché dura, l'indicizzazione tiene un `pg_advisory_lock` e l'orchestratore risponde **un messaggio generico** («sto aggiornando l'indice»): senza, la prima domanda farebbe ricaricare il modello a metà lettura. Le descrizioni delle immagini non le fa più il modello di chat (la prima immagine si riprenderebbe la VRAM appena liberata) ma un **VLM piccolo dedicato**: in sviluppo `glm-ocr` su LM Studio, 2,3 GB, che **trascrive** nomi e quantità dei prodotti (SmolVLM dentro il servizio, provato, dice solo «a few bottles»; granite-vision in GGUF è inservibile con LM Studio). Prompt in inglese e «prima trascrivi, poi descrivi»: in italiano il modello inventava. Corretti insieme due difetti che tenevano ferma tutta la catena delle immagini: mancava `generate_picture_images` (la tabella `immagini` era vuota in tutto l'indice) e il volume delle immagini era di root mentre il servizio gira come `nobody` | Una sola scheda da 16 GB per due lavori: il modello ne occupa 13, Docling circa 3. Il lock è della connessione, quindi se l'indicizzazione muore l'assistente riparte da solo, senza flag appesi. Il messaggio è generico perché lo legge chiunque: nomi di file e conteggi direbbero cosa sta entrando nelle aree altrui. In produzione cambia solo **chi** libera la VRAM (server di inferenza al posto di LM Studio), non la regola | ✅ `ingestion/indicizza.py`, `orchestratore/main.py`, test T1.18 |
| 72 | **SSO: un'istanza oauth2-proxy per servizio protetto** | In `docker-compose.yml` tre istanze distinte (`oauth2-proxy`, `oauth2-proxy-uptime`, `oauth2-proxy-litellm`), una per strumento dietro SSO (decisione 60). Ognuna ha il **suo** client Keycloak (`dagster`, `uptime`, `litellm`), callback e cookie per dominio (`dagster.localhost`, `uptime.localhost`, `modelli.localhost`), upstream dedicato e **una sola** regola di autorizzazione: Dagster e Uptime Kuma accettano `/amministratori/*`, LiteLLM solo `/amministratori/Superutente`. I commenti nel compose spiegano `--oidc-extra-audience=account` e la skip dell'issuer | Un'istanza ha una sola configurazione provider (client, callback, upstream, `ALLOWED_GROUP`) e non può differenziare l'autorizzazione per servizio; i client per-app tengono separate sessioni, log e audit | ✅ in `Sviluppo/docker-compose.yml` |
...
|---|---|---|---|
| **Immagini degli articoli nella stessa tabella** | La tabella `immagini` ha già quello che serve: chiave `(source_id, percorso)` — quindi ogni fonte possiede le sue righe anche quando due fonti di aziende diverse condividono il percorso (succede già: `erp.listini_righe` fra luis e decobrands) — più `descrizione` e `embedding` per la ricerca vettoriale. Mancano tre cose: colonna `provenienza` (documento \| articolo), perché il ciclo di vita è diverso (le figure di un documento si cancellano a ogni rilettura, quelle degli articoli seguono l'importazione); colonna `articolo` con la chiave `<azienda>:<codice>` (decisione 69); `documento` e `page` opzionali. Le ACL non cambiano: si leggono da `source_id → sources`, quindi la foto di un articolo di un'azienda resta invisibile all'altra per costruzione. Il guadagno: **una sola ricerca** per «mostrami articoli rossi», che guarda insieme le figure dei cataloghi e le foto del gestionale | **Dopo la prova del connettore ERP**: prima si vede come arrivano davvero le immagini degli articoli (URL, file, BLOB) e se hanno una descrizione utile, poi si decide la forma della tabella | `Sviluppo/ingestion/IMPOSTAZIONI.md`, migrazioni 013-014 |
| **Indicizzazione più veloce** | Misurato il 20/09/2026: ~9-13 s/pagina in GPU su cataloghi veri (13× la CPU). I due sospetti sul tempo residuo sono le descrizioni delle immagini, fatte **una alla volta** via HTTP su ~8 immagini per pagina, e l'OCR che gira su CPU mentre la GPU aspetta. Prima di ottimizzare serve un giro con i tempi separati per fase: senza, si rischia di accelerare la parte sbagliata. Interventi in ordine di resa: descrizioni in parallelo, soglia dell'area che esclude loghi e cornici, OCR su GPU, blocchi più grandi | Quando i cataloghi pesanti diventano normale amministrazione invece che eccezione. Se il corpus resta fatto di procedure da 1-2 MB, il tempo si spende meglio sulla qualità delle risposte | `Sviluppo/ingestion/PRESTAZIONI.md` |
| **Pannello: viste che si aggiornano un dato alla volta** | Oggi ogni riquadro si ridisegna intero (`innerHTML`, 65 punti nel codice): durante l'indicizzazione la barra di avanzamento lampeggia, chi scorre torna in cima, il fuoco da tastiera si perde e un nome selezionato si deseleziona. Serve una vista unica per tabella, righe con chiave stabile, e aggiornamento delle sole celle cambiate — atomico per riga, senza librerie nuove | Quando il pannello si usa davvero mentre l'indicizzazione lavora: è lì che il difetto si vede. Caso di prova: scheda Documenti di una fonte in lettura | `SPECIFICA-INTERFACCE-AMMINISTRAZIONE.md` §11c |
...
**Requisiti di design D13** (19/09/2026, registrati in `DECISIONI-APERTE.md`):

1. **Tre livelli, non sostituzioni**: Docling = *estrazione* (testo + tabelle + immagini + OCR) · Cognee = *grafo + ricerca* · gate = *permessi*. Cognee legge i formati nativi; l'OCR di Docling resta per le scansioni.
2. **Immagini fuori da Cognee**: Docling le estrae → volume con ID stabile; il VLM le descrive e la **descrizione + `[immagine:id]` entrano nel testo** di Cognee. In risposta l'orchestratore risolve l'id, passa l'immagine al VLM e il riferimento nel testo gli dice di integrarla.
3. **Archi trasversali fra aree**: grafo unico con archi anche fra aree diverse; un arco trasversale si attiva **solo** con accesso a **entrambe** le aree (regola generale). Senza accesso, arco e nodo sono **completamente invisibili** (niente «esiste ma non hai accesso», coerente con DC-09). Filtro su **nodi + archi**.
...
| **Chat gestita da un agente, ricerche come server MCP dedicati** | La chat non è più un singolo flusso riga-di-recorso: è un **agente** che, per rispondere, **chiama server MCP specifici** — `documenti` (ricerca nel RAG con ACL), `db` (domande strutturate sul gestionale/FDW), più in là calendario, posta, ecc. Ogni server MCP è il *suo* pezzo di mondo, con permessi propri; l'agente orchestra e il gate continua a decidere cosa far vedere. Il retrieval di oggi (`recupero.py`) diventa il primo server MCP, senza cambiare l'ACL | Quando la chat è in uso vero e ci sono domande che toccano più di una sorgente (documenti + anagrafiche del gestionale) | D14 |
| **Titolo automatico della conversazione** | Dopo la prima inferenza LibreChat titola la conversazione da solo (oggi l'ha già in nativo). Se non basta: titolo dal primo messaggio tramite il modello veloce, salvato con una chiamata al backend, appena finito il primo turno | Provare subito con LibreChat standard; è una riga di configurazione, non va prototipata | — |
| **Domanda generica → chiarimento mirato** | Oggi il prompt di sistema chiede UNA domanda di chiarimento solo se il CONTESTO darebbe risposte diverse secondo l'interpretazione (`prompt.py`). L'esempio "ho bisogno di un profumatore" mostra il limite: il CONTESTO risponde (ce ne sono tanti), ma manca il parametro «per che luogo / quale esigenza». Estendere: quando la domanda è generica e il conteso ha molte alternative **con attributi distinguibili** (luogo, materiale, formato), il modello fa 1-2 domande mirate **sugli attributi che filtrano davvero il recupero**, e la risposta finale filtra il CONTESTO su quegli attributi. Da fare con cura: il chiarimento costa un turno, se è generico («puoi essere più specifico?») è rumore | Dopo i primi usi veri: la reazione del modello va vista su domande reali, non sui nostri esempi | `orchestratore/prompt.py` |
